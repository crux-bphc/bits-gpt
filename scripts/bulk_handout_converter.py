...
def handout_convert(path):  # extracts only texts from the pdfs
    ...
    for i in range(0, len(texts)):
        with open(
            f"data/Handouts/rawdata/rawtext_{basename}_{i:04d}.txt",
            "w",
            encoding="utf-8",
        ) as handle:
...
def extract_tables_batch(
    handout_path, output_path
):  # extracts only tables from the pdfs in csv format
    ...
    for file in file_list:
        if file.endswith(".csv"):
            os.rename(
                os.path.join(handout_path, file),
                os.path.join(output_path, "rawtable_" + file.replace(" ", "_")),
            )
