...
def tt_chunker(input_path, output_path, chunk_size):
    ...
    with open(input_path, "r", encoding="utf-8") as handle:
        for line in handle:  # line by line read of csv file
            ...
            if (
                count == chunk_size
            ):  # if the count reaches chunk_size, save the chunk to a new csv file
                count = 0
                output_file = os.path.join(
                    output_path, f"{os.path.basename(input_path)}_chunk_{chunk:04d}.csv"
                )
...
def books_chunker(input_path, output_path):
    ...
    for index, row in df.iterrows():
        if row.iloc[0] == "COM COD":
            ...
            continue
        temp_df = pd.concat(
            [temp_df, pd.DataFrame([row], columns=header)], ignore_index=True
        )


if __name__ == "__main__":
    tt_chunker(
        r"data\Handouts\rawdata\rawtable_DRAFT_TIMETABLE_FIRST_SEMESTER_2024_-25.csv",
        r"data\Handouts\CSV",
        10,
    )
