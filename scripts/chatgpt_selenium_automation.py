...
class ChatGPTAutomation:
    ...
    def return_last_response(self):
        ...
        response_elements = self.driver.find_elements(
            by=By.XPATH,
            value='//div[@class="markdown prose w-full break-words dark:prose-invert dark"]',
        )
    ...
    for raw_file in raw_data_filelist:
        ...
        with open(os.path.join(in_path, raw_file), "r", encoding="utf-8") as handle:
            ...
            raw_data = raw_data.replace(
                "\n", "(newline)"
            )  # this and the next line is to not fuck up the javascript syntax
            raw_data = raw_data.replace("'", "\\")
            ...
            if "rawtable" in raw_file:
                ...
                prompt = (
                    input_prompt + f"|| use ONLY the following CSV data : {raw_data} ||"
                )
            try:
        ...
        shutil.move(
            os.path.join(os.getcwd(), in_path, raw_file),
            os.path.join(os.getcwd(), processed_path),
        )
